...
contract('Airline', accounts => {
    ...
    it('should get the real balance of the contract', async() => {
        let fligth = await instance.flights(0);
        let price = fligth[1];

        let fligth2 = await instance.flights(1);
        let price2 = fligth2[1];
    ...
    it('should allow customers to rendeem loyalty points', async() => {
        ...
        let loyaltyPoint = customer[0];

        assert(loyaltyPoint, 0);
